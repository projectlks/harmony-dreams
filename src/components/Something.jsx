...
function Card({ title, paragraph, ...props }) {
  ...
  return (
    <div data-aos="slide-left" className="w-full h-auto">
      <div
        {...props}
        className="flex flex-col h-full shadow-darkBlue transition-all shine-overlay w-[90%] mx-auto md:w-full border-2 border-[#AEAEAE] hover:border-hidden bg-gradient-to-tr hover:from-[#d9d9d9] hover:to-[#AEAEAE] hover:shadow-md hover:scale-110 cursor-pointer rounded-md px-5 pb-10 py-8 items-center group select-none
  ...
  "
      >
        <div className="w-auto mx-auto p-4 bg-[#D9D9D9] rounded-full">
          <img src={imgSrc} className={`h-auto mx-auto ${props.className}`} alt={title} />
        </div>
